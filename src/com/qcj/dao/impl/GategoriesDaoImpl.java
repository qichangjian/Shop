...
public class GategoriesDaoImpl implements GategoriesDao {
	...
	public List<Gategories> selectPageGategories(int pageOn, int pageNumber) {
		...
		List<Gategories> list = jdbcTemplate.query(sql, new Object[]{pageOn*pageNumber,(pageNumber-1) * pageOn},
				((RowMapper<Gategories>)(rs,index) -> new Gategories(rs.getInt(1),rs.getString(2),rs.getString(3))));
	...
	public int addGategories(Gategories gategories) {
		...
		int result = jdbcTemplate.update(sql,new Object[] {gategories.getGategoriesName(),gategories.getGategoriesMessage()});
		return result;
	}

	@Override
	public List<Gategories> selectPageGategories(int pageOn, int pageNumber, String gategoriesNameLike) {
		String sql=" select * from t_gategories where rowid in(select rid from (select rownum rn,rid from(select rowid rid,gategoriesId from t_gategories where gategoriesName like '%"+gategoriesNameLike+"%') where rownum<=?) where rn>?)";
