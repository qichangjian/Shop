...
public class UserDaoImpl implements UserDao {
	...
	public List<User> selectPageUser(int pageOn, int pageNumber) {
		...
		List<User> list = jdbcTemplate.query(sql, new Object[]{pageOn*pageNumber,(pageNumber-1) * pageOn},
				((RowMapper<User>)(rs,index) -> new User(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5))));
	...
	public List<User> selectPageUser(int pageOn, int pageNumber, String userNameLike) {
		String sql=" select * from t_user where rowid in(select rid from (select rownum rn,rid from(select rowid rid,userId from t_user where userName like '%"+userNameLike+"%') where rownum<=?) where rn>?)";
		System.out.println("模糊查询canshu :" +pageOn+":"+pageNumber+"计算后-"+ pageOn*pageNumber+":"+(pageNumber-1) * pageOn);
		List<User> list = jdbcTemplate.query(sql, new Object[]{pageOn*pageNumber,(pageNumber-1) * pageOn},
				((RowMapper<User>)(rs,index) -> new User(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(4))));
	...
	public int addUser(User user) {
		...
		int result = jdbcTemplate.update(sql,new Object[] {user.getUserName(),user.getUserEmail(),user.getUserPassword(),user.getUserPhone()});
		return result;
	}
