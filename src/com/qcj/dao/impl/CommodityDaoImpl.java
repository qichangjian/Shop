...
public class CommodityDaoImpl implements CommodityDao {
	...
	public int allCountNum(String commodityNameLike, int gategoriesId) {
		String sql = "select count(*) from t_commodity where gategoriesId = " + gategoriesId
				+ " and commodityName like '%" + commodityNameLike + "%'";
	...
	public List<Commodity> selectPageCommodity(int pageOn, int pageNumber, String commodityNameLike, int gategoriesId) {
		String sql = "select * from t_commodity where rowid in(select roid from (select rownum rn,roid from(select rowid roid ,commodityId from t_commodity where gategoriesId = "
				+ gategoriesId + " and commodityName like '%" + commodityNameLike + "%') where rownum<=?) where rn>?)";
		List<Commodity> list = jdbcTemplate.query(sql, new Object[] { pageOn * pageNumber, (pageNumber - 1) * pageOn },
				((RowMapper<Commodity>) (rs, index) -> new Commodity(rs.getInt(1), rs.getInt(2), rs.getString(3),
	...
	public int addCommodity(Commodity commodity) {
		...
		int result = jdbcTemplate.update(sql,
				new Object[] { commodity.getGategoriesId(), commodity.getCommodityName(), commodity.getCommodityPrice(),
						commodity.getCommodityIsMan(), commodity.getCommodityIsShelves(),
						commodity.getCommoditySize() });
		return result;
	}
	...
	public int commodityUpdate(int commodityId, String commodityName, String commodityPrice, String commodityIsMan,
		...
		int result = jdbcTemplate.update(sql, new Object[] { commodityName, commodityPrice, commodityIsMan,
				commodityIsShelves, commoditySize, commodityId });
		return result;
	}
	...
	public List<Commodity> selectCommodityBycId(int gategoriesId) {
		String sql = "select * from t_commodity where gategoriesId = ?";
		List<Commodity> list = jdbcTemplate.query(sql, new Object[] { gategoriesId },
				((RowMapper<Commodity>) (rs, index) -> new Commodity(rs.getInt(1), rs.getInt(2), rs.getString(3),
						rs.getLong(4), rs.getString(5), rs.getString(6), rs.getString(7))));
		System.out.println("sql list.size:" + list.size());
		return null == list || list.size() == 0 ? null : list;
