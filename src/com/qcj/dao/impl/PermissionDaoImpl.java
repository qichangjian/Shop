...
public class PermissionDaoImpl implements PermissionDao {
	 ...
	 */
	/*
	 * @Override public List<Permission> selectPagePermission(int pageOn, int
	 * pageNumber) { String
	 * sql="select * from t_permission where rowid in(select rid from (select rownum rn,rid from(select rowid rid,pid from t_permission) where rownum<=?) where rn>?)"
	 * ; // List<Permission> list = jdbcTemplate.query(sql, new
	 * Object[]{pageOn*pageNumber,(pageNumber-1) * pageOn}, //
	 * ((RowMapper<Permission>)(rs,index) -> new
	 * Permission(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4)))
	 * ); // System.out.println("sql list.size:"+list.size()); // return
	 * null==list||list.size()==0?null:list; return null; }
	 */

	public List<Map<String, Object>> queryPermission(int pageOn, int pageNumber) {
		String sql = "select * from t_permission where rowid in(select rid from (select rownum rn,rid from(select rowid rid,pid from t_permission) where rownum<="
				+ pageOn * pageNumber + ") where rn>" + (pageNumber - 1) * pageOn + ")";
		return jdbcTemplate.queryForList(sql);
	}

	public List<Map<String, Object>> queryPermission(int pageOn, int pageNumber, String permissionNameLike) {
		String sql = "select * from t_permission where rowid in(select rid from (select rownum rn,rid from(select rowid rid,pid from t_permission where pname like '%"
				+ permissionNameLike + "%') where rownum<=" + pageOn * pageNumber + ") where rn>"
				+ (pageNumber - 1) * pageOn + ")";
		return jdbcTemplate.queryForList(sql);
	}
	...
	public int permissionUpdate(int permissionId, String permissionName, String permissionUrl,
		...
		System.out.println(" ------dao canshu:" + permissionId + ":" + permissionName + ":" + permissionUrl + ":"
				+ permissionMessage);
		int result = jdbcTemplate.update(sql,
				new Object[] { permissionName, permissionUrl, permissionMessage, permissionId });
		return result;
	}
